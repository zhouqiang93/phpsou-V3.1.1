...
switch ($action)
{
	...
	case "del":
	     $id=$_GET["id"];
		 mysql_query("delete from phpsou_url where url='".$id."'");
	break;
}

$site="select * from phpsou_url";
...
function saveconfig()
{
	...
	if($id<>'')
	{
	mysql_query("update phpsou_url set url='$url',site='$site' where url='$id'");
	jsalert("修改成功","url.php");}
	else
	{mysql_query("insert into phpsou_url(url,site) value ('$url','$site')");jsalert("添加成功","url.php");}
