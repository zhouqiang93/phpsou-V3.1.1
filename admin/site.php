...
switch ($action)
{
	...
	case "del":
	     $id=$_GET["id"];
		 mysql_query("delete from phpsou_site where url='".$id."'");
	break;
}

$site="select * from phpsou_site";
...
{ 
   $arr[]=array("id"=>$row['id'],"url"=>$row['url']); 
} 
...
function saveconfig()
{
	...
	if($id<>'')
	{
	mysql_query("update phpsou_site set url='$url',site='$site' where url='$id'");
	jsalert("修改成功","site.php");}
	else
	{mysql_query("insert into phpsou_site(url,site) value ('$url','$site')");jsalert("添加成功","site.php");}
